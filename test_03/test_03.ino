...
void update2() {
  ...
  for (int column = 0 ; column < NUM_COLS; column++) {    
    ...
    for (int panel = 0; panel < NUM_PANELS; panel++) {  // para cada panel tengo que pasarle 4 bytes
      panels[column + (NUM_COLS * panel)] =  0 ;      // reset

      panels[column + (NUM_COLS * panel)] |=  long(B1) ;      
      panels[column + (NUM_COLS * panel)] |=  long(B00001001) << 8;  
      panels[column + (NUM_COLS * panel)] |=  long(reverse(B00001001)) << 16;
      panels[column + (NUM_COLS * panel)] |=  long(reverse(B1)) << 24;     
